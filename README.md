...
| `UNUSED_METHOD` | A **private** method has no identifier reference anywhere in its file outside its own body (a recursive call does not keep it alive) and its name appears in no string literal or resource. Constructors, `native` methods, and the serialization hooks (`readObject`, `writeReplace`, ...) are never reported. Annotated members are kept. | Removes the member, its attached comments, and imports that only it needed. |
...
reported. Besides Java files, a class name counts as referenced when it appears in a text
...
- `terminal` (default local output): one `- [SEVERITY] path[:line[:column]] :: message` line per finding, then a `Summary:` line
...
- `2`: usage error (unknown command, unknown option, unsupported `--format`, `--root` is not a directory, empty or invalid `--exclude`)
- `3`: the analysis or rewrite failed (unreadable root, file changed during `fix`)

### Maven plugin

Once `./mvnw install` has published it to the local repository (the plugin runs on Maven 3.9 and
Maven 4):
...
(`prune.excludes`, comma-separated globs), `skip` (`prune.skip`). Both goals skip `pom`-packaging
modules, so in a reactor each module is analyzed exactly once. `fix` rewrites the module's sources
and `pom.xml` and logs what remains.

### Gradle plugin

The plugin is not published to a remote repository. Once `cd prune-gradle-plugin && ./gradlew
publishToMavenLocal` has installed it and its plugin marker into the local repository, let
`settings.gradle` resolve plugins from there:
