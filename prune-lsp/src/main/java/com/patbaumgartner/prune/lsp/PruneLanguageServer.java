...
public final class PruneLanguageServer implements LanguageServer, LanguageClientAware {
    ...
    static final List<String> WATCHED_GLOBS = List.of("**/*.java", "**/pom.xml", "**/build.gradle", "**/build.gradle.kts");
    ...
    // Clients only report changes for watchers the server registers; without one, deleting a
    // Java file or editing pom.xml would leave stale diagnostics until the next save.
    ...
    public TextDocumentService getTextDocumentService() {
        return new TextDocumentService() {
            ...
            public CompletableFuture<List<Either<Command, CodeAction>>> codeAction(CodeActionParams params) {
                ...
                return CompletableFuture.completedFuture(actions.stream().map(Either::<Command, CodeAction>forRight).toList());
            }
        };
    }
    ...
    synchronized void refreshDiagnostics() {
        ...
        try {
            ...
            client.logMessage(new MessageParams(MessageType.Error, "prune-java could not analyze " + projectRoot
                    + ": " + failure.getMessage()));
            return;
        }
        lastReport = report;
