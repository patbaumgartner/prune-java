...
For a first contribution, make a focused change, run the relevant test, then run both builds before
opening a pull request. Use the [test conventions](.github/instructions/tests.instructions.md) when
adding tests and the [sample guide](samples/README.md) when changing analyzer behavior.

## Quick reference

| Change | Start here | Verify with |
| --- | --- | --- |
| Analyzer rule or guard | `prune-core` and its unit tests | `./mvnw -pl prune-core test` and sample manifests |
| CLI, Maven, or LSP adapter | The matching module | `./mvnw install` |
| Gradle plugin | `prune-gradle-plugin` | `./mvnw install`, then `cd prune-gradle-plugin && ./gradlew build` |
| Documentation only | Relevant Markdown or `.github` guidance | Check links and commands; run `actionlint` when changing workflows |

The full build commands below include integration tests. For a faster test loop, run the focused
module test first; before submitting, run both full builds because the Gradle build consumes the
locally installed `prune-core` artifact.
...
git commit -am "chore(release): release 1.0.0" && git tag v1.0.0 && git push --tags
./mvnw versions:set -DnewVersion=1.1.0-SNAPSHOT -DgenerateBackupPoms=false
git commit -am "chore(release): prepare 1.1.0-SNAPSHOT" && git push
```
