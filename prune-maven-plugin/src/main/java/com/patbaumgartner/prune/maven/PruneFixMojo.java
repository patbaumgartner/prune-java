...
public final class PruneFixMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        try {
            AnalysisReport report = new ConservativeUnusedCodeAnalyzer().analyze(MojoSupport.configFor(projectDir, excludes));
            AnalysisReport remaining = new SourceAutofixEngine(root).fix(report);
            getLog().info("prune-java fix: " + remaining.summary());
            MojoSupport.logReport(getLog(), new ReportRenderer().render(remaining, outputFormat), remaining.issueCount() > 0);
