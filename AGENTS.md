...
- **CLI exit codes**: `0` success, `1` `check` found issues or `fix` left issues that need a human, `2` usage error (unknown command, unknown option, unsupported `--format`, `--root` not a directory), `3` analysis or rewrite failure.
