...
public abstract class PruneCheckTask extends DefaultTask {
	...
	// Internal rather than an input file: the file is allowed to be absent.
	@Internal
	...
	public void runCheck() {
		...
		try {
			report = new ConservativeUnusedCodeAnalyzer().analyze(TaskSupport.configFor(getProjectRoot(), getExcludes(),
					getBaseline(), getTestReferences(), getExplain().get()));
		}
		catch (UncheckedIOException | IllegalArgumentException exception) {
			throw new GradleException(
					"prune-java could not analyze " + getProjectRoot().get() + ": " + exception.getMessage(),
					exception);
		}
