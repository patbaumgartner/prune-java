...
@DisableCachingByDefault(because = "Rewrites the project's sources in place and declares no outputs")
public abstract class PruneFixTask extends DefaultTask {
	...
	public void runFix() {
		...
		try {
			AnalysisReport report = new ConservativeUnusedCodeAnalyzer().analyze(
					TaskSupport.configFor(getProjectRoot(), getExcludes(), getBaseline(), getTestReferences(), false));
			AnalysisReport remaining = new SourceAutofixEngine(root).fix(report);
			getLogger().lifecycle("prune-java fix: {}", remaining.summary());
			TaskSupport.logReport(getLogger(), new ReportRenderer().render(remaining, format),
