...
final class LineEdits {
    ...
    // Whole-line deletion of firstLine..lastLine (one-based, inclusive) plus directly attached
    // leading comment lines. One neighbouring blank line goes too when the removal would otherwise
    // leave a doubled blank line, a blank line before a closing brace or tag, or a blank line
    // right after an opening one.
    static int[] deleteLines(String content, LineMap map, int firstLine, int lastLine, Predicate<String> attachedComment) {
        int first = firstLine;
