...
// Removing a member can leave an import behind that nothing else in the file needs. Such an import
// is removed with the member so that builds enforcing unused-import rules keep passing. Imports that
// were already unused before the fix are left alone: they are not this fix's business.
final class ImportCleanup {
    ...
    static List<FixAction> plan(Path file, String content, List<FixAction> memberActions) {
        ...
        // Orphaned imports on consecutive lines go as one block so the blank-line handling sees the
        // whole gap instead of leaving one empty line per removed import behind.
