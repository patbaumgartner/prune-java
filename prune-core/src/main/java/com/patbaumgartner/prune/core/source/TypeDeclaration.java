...
public record TypeDeclaration(
    ...
    // Since Java 25 a launcher also accepts non-static `void main()`; only a private main is unreachable.
    public boolean hasMainMethod() {
        return members.stream().anyMatch(member -> member.kind() == MemberDeclaration.MemberKind.METHOD
                && member.name().equals("main")
                && !member.isPrivate());
    }
