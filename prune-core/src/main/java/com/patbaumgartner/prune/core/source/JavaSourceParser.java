...
public final class JavaSourceParser {
    ...
    public static JavaSourceFile parse(String relativePath, String content) {
        ...
        try {
            // Outside literals and comments a backslash can only start a unicode escape, which this
            // parser does not translate, so an escaped identifier letter would hide a real reference.
    ...
    private void parseCompilationUnit() {
        ...
        while (peek().isKeyword("import")) {
            next();
            if (peek().isKeyword("static") || (peek().isIdentifier("module") && peek(1).kind() == TokenKind.IDENTIFIER)) {
                next();
            }
    ...
    private TypeDeclaration parseType(String enclosingName) {
        ...
        return new TypeDeclaration(kind, name.text(), modifiers, annotated, documentedStarts.contains(start), superTypes,
                start, name.start(), publicOffset, lastEnd(), members, nested);
    }
    ...
    // `<` in an expression is either a comparison or the start of type arguments. At declarator
    // depth, type arguments can only follow `new Type`, `instanceof Type`, an explicit witness
    // `Type.<T>method`, or precede a method reference `Type<T>::name`; anything else, such as
    // `p = a < b, q = c > d`, is a comparison whose comma ends the declarator.
    ...
    // Offset from pos of the `>` that closes the run starting at pos, or -1 when the run contains
    // anything that cannot appear inside type arguments.
        ...
        for (int ahead = 0; ahead < 64; ahead++) {
            ...
            if (token.isPunctuation('<')) {
                ...
                if (depth == 0) {
                    ...
                    || token.text().equals("extends") || token.text().equals("super")))
    ...
    private Token tokenBeforeQualifiedChain(int identifierIndex) {
        ...
        while (index >= 2 && tokens.get(index - 1).isPunctuation('.') && tokens.get(index - 2).kind() == TokenKind.IDENTIFIER) {
            index -= 2;
        }
