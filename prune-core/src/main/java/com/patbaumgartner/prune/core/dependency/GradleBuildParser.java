...
public final class GradleBuildParser {
    ...
    private static final Pattern STRING_NOTATION = Pattern.compile(
            "^\\s*" + CONFIGURATIONS + "\\s*\\(?\\s*(['\"])([^'\"]+)\\2\\s*\\)?\\s*(\\{.*)?$");
    private static final Pattern MAP_NOTATION = Pattern.compile(
            "^\\s*" + CONFIGURATIONS + "\\s*\\(?\\s*group\\s*[:=]\\s*(['\"])([^'\"]+)\\2\\s*,\\s*name\\s*[:=]\\s*(['\"])([^'\"]+)\\4.*$");
    ...
    public static List<DeclaredDependency> parse(String buildFile, String script) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (string.matches()) {
                ...
                DeclaredDependency dependency = fromCoordinates(string.group(1), string.group(3), buildFile, i + 1, endLine);
                if (dependency != null) {
    ...
    // One-based line on which the braces opened on line `from` close again; the line itself if
    // none are left open, so a dangling block never yields a partial deletion span.
    ...
    private static DeclaredDependency fromCoordinates(String configuration, String notation, String buildFile, int line,
        ...
        return new DeclaredDependency(parts[0], parts[1], scopeOf(configuration), "", classifier, buildFile, line, line, endLine);
    }
