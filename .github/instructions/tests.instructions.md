...
- **`prune-cli`** — `PruneCliApplication` has a package-private constructor taking `UnusedCodeAnalyzer` and `ReportRenderer`, and a package-private `run(String[], PrintStream, PrintStream)`. Drive it with `ByteArrayOutputStream` and assert on the exit code plus captured stdout/stderr. Never call `main`; it calls `System.exit`.
- **`prune-lsp`** — `PruneLanguageServer` has a package-private constructor taking `UnusedCodeAnalyzer` (a lambda returning a synthetic `AnalysisReport` is enough). `connect` the `RecordingClient implements LanguageClient` from `PruneLanguageServerTest`, which lists every `publishDiagnostics` and `logMessage` call, then invoke `initialize` / `initialized` / `didSave` directly. Never go through `PruneLspApplication.main` or `LSPLauncher`; `main` calls `System.exit`. lsp4j 1.0.0 wraps `Diagnostic.getMessage()` and `getCode()` in `Either`, so assert on `.getLeft()`.
...
- `unused` — what the analyzer **must eventually report**. Not asserted yet; the analyzer is still a scaffold. These become the acceptance criteria as detection lands.
- `kept` — what the analyzer **must never report**. Asserted today and forever, because a false positive that deletes live code is the failure mode this project exists to avoid.

When you add detection, convert `unused` rows into assertions rather than editing them to match whatever the analyzer happens to emit. Adding a symbol to a sample means adding its row to the manifest in the same change; `SampleProjectFixtureTest` fails on an unknown verdict or issue type, so typos surface immediately.

Because the scaffold reports nothing, the `kept` guard is currently vacuous. `keptSymbolGuardFailsWhenAReportNamesAKeptSymbol` feeds a synthetic report through the same helper to prove the guard still fails when it should — keep an equivalent test alive until real detection makes the guard self-evidently live.

## Integration tests

Unit tests cannot catch goal- and task-invocation failures, so both plugins are also driven end to end:

- **Maven** — `maven-invoker-plugin` runs the packaged plugin's `check` and `fix` goals against `samples/maven-sample` in a real Maven process during `install`. This is what catches a `@Parameter` whose declared type Maven cannot inject. Skip with `-Dinvoker.skip=true`. Logs land in `prune-maven-plugin/target/it/maven-sample/build.log`.
- **Gradle** — `SampleProjectFunctionalTest` copies `samples/gradle-sample` into a `@TempDir`, adds the plugin id to its `plugins {}` block, and runs `pruneCheck` / `pruneFix` through TestKit.
